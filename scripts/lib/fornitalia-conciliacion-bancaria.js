...
(function (global) {
  ...
  function grupoDesdeDescarte(row) {
    ...
    return {
      ...
      descartado_by: row.descartado_by,
      descRow: row
    };
  }
  ...
  function canalCajaSinFactura(caja, moneda) {
    ...
    if (t.indexOf('dolar') >= 0 || t.indexOf('dollar') >= 0 || /\busd\b/.test(t) || mon === 'USD') return 'efectivo_sf_usd';
    return 'efectivo_sf';
  }
  ...
  function parseTesoreriaMp(wb, archivo, hoja) {
    ...
    for (var r = 1; r < rows.length; r++) {
      ...
      var esUsdSf = canalSf === 'efectivo_sf_usd';
      var filaComun = {
        origen_id: origenId,
        fecha: fecha || fechaHoyYmd(),
        fecha_hora: fechaHora,
      ...
      if (canalSf) {
        filasCajaSf.push(Object.assign({}, filaComun, {
          ...
          raw: {
            ...
            id: idCierre || null, moneda: esUsdSf ? 'USD' : (monedaFila || 'ARS'),
    ...
    if (!filas.length) {
      if (filasCajaSf && filasCajaSf.length) {
  ...
  async function onUpload(origen) {
    ...
    pedirArchivo(esCanalExtractoBanco(state.canal) ? acceptGal : acceptXlsx, async function (file) {
      ...
      try {
        ...
        var ingestSf = { n: 0, canales: [] };
        if (origen === 'sistema' && (parsed.filasCajaSf || []).length) {
          if (window.FornitaliaCajasFisicas && typeof window.FornitaliaCajasFisicas.ingestSinFactura === 'function') {
            ingestSf = await window.FornitaliaCajasFisicas.ingestSinFactura(parsed.filasCajaSf, {
              archivo: file.name,
              formatoCierre: parsed.formatoCierre
            });
            if (ingestSf.err) throw new Error(ingestSf.err);
          }
        }
  ...
  function valSolo(m, key) {
    ...
    if (key === 'usuario') {
      return {
        v: textoUsuario(m.no_requiere_by, m.eliminado_by, m.updated_by, m.created_by),
        t: 'txt'
      };
    }
  ...
  function valAnulado(p, key) {
    ...
    if (key === 'usuario') return { v: textoUsuario(p.a && p.a.created_by), t: 'txt' };
  ...
  function renderTablaAnulados() {
    ...
    rows.forEach(function (p) {
      html += '<tr>' +
        ...
        '<td>' + esc(p.opRel || '—') + '</td>' +
        '<td>' + htmlUsuario(p.a && p.a.created_by) + '</td>' +
        '<td class="cb-col-acc">' +
    ...
    if (esMatch) {
      ...
      matches.forEach(function (m) {
        ...
        aoa.push([
          ...
          idsOrigenGrupo(ss),
          textoUsuario(m.confirmado_by)
        ]);
      });
    } else if (state.lista === 'anulados') {
      aoa.push(['Fecha', 'Movimiento', 'Importe', 'Fecha anulación', 'Anulación', 'Importe anulación', 'Operación relacionada', 'ID movimiento', 'ID anulación', 'Usuario']);
      dateCols = [0, 3];
      numCols = [2, 5];
      ...
      pares.forEach(function (p) {
        aoa.push([
          ...
          textoUsuario(p.a && p.a.created_by)
        ]);
      });
