...
- **Saldos extractos** (`eb_saldo_extracto.fecha_desde` / `fecha_hasta`): período del resumen de cuenta (Galicia PDF), carta MP y cortes de cajas físicas Efectivo-f (ARS), Morba-s/f (ARS) y Efectivo-f (USD) en calendario Argentina. Defaults `public.fecha_hoy_argentina()`. El gráfico y los filtros agrupan por mes de `fecha_hasta`.
- **Cajas (físicas)** (`cf_movimiento.fecha`): día de negocio Argentina desde el Excel de tesorería/cierre (dd/mm/aaaa, ISO o serial de día) de Efectivo-f (ARS), Morba-s/f (ARS) o Efectivo-f (USD). El corte de saldo que alimenta Saldos extractos usa min/max de esas fechas.
