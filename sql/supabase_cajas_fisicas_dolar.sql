-- Cajas (físicas): Efectivo-f (USD) = efectivo dólar (tesoreria_efectivo_dolar / cierre_DOL).
-- Los importes se persisten en ARS, pesificados al usd_mep de tipo_de_cambio
-- (fecha del movimiento o última cotización anterior).

ALTER TABLE public.cf_movimiento
  ADD COLUMN IF NOT EXISTS monto_usd numeric(18,2),
  ADD COLUMN IF NOT EXISTS tipo_cambio_mep numeric(18,6),
  ADD COLUMN IF NOT EXISTS tipo_cambio_fecha date;

COMMENT ON COLUMN public.cf_movimiento.monto_usd IS
  'Importe original en USD (solo Efectivo-f USD). monto/crédito/débito/saldo quedan en ARS.';
COMMENT ON COLUMN public.cf_movimiento.tipo_cambio_mep IS
  'usd_mep usado para pesificar (día del movimiento o último anterior).';
COMMENT ON COLUMN public.cf_movimiento.tipo_cambio_fecha IS
  'Fecha de la cotización MEP aplicada (puede ser anterior al movimiento).';
...
ALTER TABLE public.cf_movimiento
  ...
  CHECK (canal IN ('galicia_facturada', 'morba_sf', 'galicia_dolar'));
...
ALTER TABLE public.eb_saldo_extracto
  ...
  CHECK (canal IN ('galicia', 'mercadopago', 'galicia_facturada', 'morba_sf', 'galicia_dolar'));

COMMENT ON TABLE public.cf_movimiento IS
  'Movimientos de cajas físicas (no conciliables). Efectivo-f (ARS) = efectivo pesos; Morba-s/f (ARS) = Transferencia Morba; Efectivo-f (USD) = efectivo dólar pesificado a ARS con MEP.';

COMMENT ON TABLE public.eb_saldo_extracto IS
  'Un renglón por corte de saldo. Galicia PDF, Mercado Pago Carta de saldo y cajas físicas Efectivo-f (ARS), Morba-s/f (ARS) y Efectivo-f (USD, saldo en ARS). Upsert por canal+cuenta+fecha_hasta.';
...
BEGIN
  ...
  WHERE NULLIF(btrim(x->>'fecha_hasta'), '') IS NOT NULL
    AND COALESCE(NULLIF(btrim(x->>'canal'), ''), 'galicia') IN (
      'galicia', 'mercadopago', 'galicia_facturada', 'morba_sf', 'galicia_dolar'
    )
...
BEGIN
  ...
  END IF;
  IF p_canal IS NULL OR p_canal NOT IN ('galicia_facturada', 'morba_sf', 'galicia_dolar') THEN
...
BEGIN
  ...
  FROM jsonb_array_elements(COALESCE(p_filas, '[]'::jsonb)) AS x
  WHERE NULLIF(btrim(x->>'fecha_hasta'), '') IS NOT NULL
    AND COALESCE(NULLIF(btrim(x->>'canal'), ''), 'galicia_facturada') IN (
