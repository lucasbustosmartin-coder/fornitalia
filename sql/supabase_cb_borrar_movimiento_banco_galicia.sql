-- Eliminar un movimiento del extracto Galicia (solapa Solo banco).
-- No toca Mercado Pago. Requiere sql/supabase_conciliacion_bancaria.sql
-- y sql/supabase_conciliacion_bancaria_manual_grupo.sql.
...
BEGIN
  ...
  END IF;
  IF v_mov.origen <> 'banco' OR v_mov.canal <> 'galicia' THEN
    RAISE EXCEPTION 'Solo se pueden eliminar movimientos del extracto de Banco Galicia (solapa Solo banco).';
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.cb_borrar_movimiento_banco_galicia(uuid) FROM PUBLIC;

COMMENT ON FUNCTION public.cb_borrar_movimiento_banco_galicia(uuid) IS
  'Elimina un movimiento del extracto Galicia que no está en sugerido/confirmado. Permiso cargar_conciliacion_bancaria.';
