...
BEGIN
  ...
  SET estado = p_estado,
      ...
      confirmado_by = CASE WHEN p_estado = 'confirmado' THEN auth.uid() ELSE confirmado_by END
  WHERE id = p_match_id;
END;
$$;
...
COMMENT ON TABLE public.cb_match IS
  'Parejas sugeridas o confirmadas entre un movimiento de banco y uno de tesorería, por canal.';
...
COMMENT ON FUNCTION public.cb_set_match_estado(uuid, text) IS
  'Confirma o descarta un match (confirmado / rechazado).';
